...
def Get_keyword(sentence):
    ...
    key_word_noun = komoran.nouns(key_word) # 핵심 단어에서 명사만 추출 (리스트)
...
def frequent_list(sentence, word_num):
    komoran = Komoran()
    noun_list = komoran.nouns(sentence)    # 명사만 추출

    for noun in noun_list:  # 한 글자인 단어는 제거
        if len(noun) < 2:
