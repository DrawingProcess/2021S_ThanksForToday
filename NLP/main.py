...
if __name__ == '__main__':
    sent = "오늘은 모모의 외박 둘째날이자 마지막날~~~ 전에 먹었던 누메로도스의 마스카포네바질피자가 그리워져서 점심으로 먹기위해 뚝섬역에서 만났당! 아참 어제 연보라색으로 염색하고 머리한번 감았더니 머리가 고냥 코토리베이지가 되버려따^-^ㅋㅋㅋㅋㅋ 푸른빛 연보라색이라 좀 맘에안들었는데 내가좋아하는 베이지색이 되서 사실 넘좋았당ㅋㅋㅋㅋㅋㅋ 누메로도스는 다른거보다 피자맛집이라서 바질피자랑 양배추찐게 들어간 새로운피자 하나를 도전해봣는데 나는완전별로였고 연구는 맘에들어했다ㅋㅋㅋㅋ 바질피자는 여전히 존맛..😋 다먹고 그근처를 걷는데 너무 예뻤다!!! 우리가 좋아하는 상수의 분위기 상위버전이랄까! 가게 하나하나가 되게 고급스럽거나 독특하고 분위기가 최고였당☺️ 나중에 놀러오기로했는데 뚝섬역 근처에 놀곳이나 이어져있는곳이 딱히 없어서 안갈것같긴하다ㅋㅋㅋㅋ 애견카페를 가기 위해 어린이대공원역에 있는 히릿~!~!을 갔당 우리가 좋아하던 불독은 없어졌지만ㅠㅠㅠ 전에 봣던 아가들이 꽤있었당 다른애견카페들은 간식에 맛들려서 간식만 찾아다니는 영악(?)한 애들이 많은데 히릿은 간식만 찾아다니지않고 간식이 없어도 곁에 와주는 댕댕이들이 많아서 넘좋당! 올티닮은 시츄랑 다리하나가 없는 하얀포메닮은아이가 우리가 갈때까지 주구장창 우리곁을 지켰다ㅋㅋㅋㅋㅋ 귀여웡😆 글고 닥스훈트 닮은 쪼꼬미도 우리한테 자주왔는데 걸레 담아놓은 바구니에 올라가서 자리잡고 자는거 넘귀여워따ㅋㅋㅋㅋ 배고파져서 모모 자취방에가서 떡볶이를 시켜먹기로했다! 떡볶이 오는 동안 모모가 준비한 내 생일선물 개봉식을 했당!! 요즘 빈티지 패션에 빠져서 목걸이,팔찌,신발 이런 것들을 막 찾아보고 사고싶어했는데 연구가 딱 빈티지 컬렉션으로다가 준비를 해뒀당ㅋㅋㅋㅋ 보헤미안 느낌 뿜뿜나는 목걸이랑 팔찌랑 노트북파우치, 베이지색 플랫슈즈, 노트북에 붙일 힙한스티커, 그리고 편지가 있었당! 히히 오랜만에 써준 편지도 좋았구 선물들이 다 너무 맘에들었다><😘 옷도 사고싶었다는데 옷은 내취향이 너무깐깐해서 안샀다고함 ㅋㅋㅋㅋㅋㅋ 옷은 내가 사는게 낫긴함 ㅎㅎ 취향저격 선물개봉식을 마치고 오랜만에 배달떡볶이를 먹으면서 응팔을 봣다ㅋㅋㅋ 애견카페를 갔다와서 그런지 어제보다 더 알찬 하루였다ㅎㅎ 히히 재밌었당 뿅!🥳❣"
    #sentence = "디지털 뉴딜(Digital New Deal)이 화두로 떠오르고 있다. 디지털 뉴딜이란 코로나19 사태 이후 경기 회복을 위해 마련된 범국가적 프로젝트다. 국내 산업의 디지털화를 가속하고 비대면화를 촉진시켜 디지털 기반의 일자리 창출 및 경제 혁신 가속화를 추진한다는 의미다. 이처럼 코로나19 사태를 극복할 새로운 원동력으로 디지털 뉴딜 정책이 각광을 받고 있는 상황에서 지난 3일 전남대학교에서는 포스트 코로나 시대, 전환포럼 2020이 개최되어 주목을 끌었다. 포스트 코로나 시대, 과학기술의 역할과 한국형 뉴딜 실현이라는 주제로 대통령직속 정책기획위원회가 주관한 이번 행사는 코로나19 사태 이후 전개될 시대를 전환적 관점에서 바라보면서 새로운 미래와 성장의 기회를 모색하자는 취지로 마련됐다. 포스트 코로나 시대, 디지털 뉴딜 정책 방향에 대해 발제를 맡은 고진 4차산업혁명위원회 위원은 디지털 뉴딜을 코로나19 사태로 인해 벌어진 경제 위기를 극복하기 위한 우리만의 D.N.A 기반 회복 전략이라고 정의했다. D.N.A란 데이터(Data)와 네트워크(Network), 그리고 인공지능(AI)을 뜻한다. 뉴딜(New Deal) 정책은 세계 대공황 시절에 미국의 루스벨트 대통령이 위기 극복을 위해 채택한 정책이다. 대규모 공공 토목사업을 통해 일자리를 창출하는 정책으로서 당시의 핵심 사업장은 후버댐 건설이었다. 디지털 뉴딜은 미국의 뉴딜과 비슷하면서도 다르다. 유사한 점은 정책 추진을 통해 대규모 일자리를 창출한다는 점이고, 다른 점은 토목사업이 아닌 디지털 기술을 통해 일자리를 만든다는 점이다. 뉴딜 정책의 핵심 사업장이 후버댐이라면, 디지털 뉴딜의 핵심사업은 데이터댐이라는 것이 고 위원의 설명이다. 데이터댐이란 공공기관이나 민간기업이 데이터를 수집하고, 이를 가공하여 쓸모 있는 정보로 재구성한 집합 시스템을 가리킨다. 이를 활용하면 더 스마트한 AI를 개발할 수 있고, 기존의 굴뚝산업을 혁신산업으로 변신시킬 수도 있다. 데이터댐을 구축하려면 데이터를 수집하고 이를 표준화시켜야만 한다. 또한 데이터를 가공하거나 결합시켜 새로운 데이터를 만들어내기도 한다. 이 과정은 모두 사람의 힘을 통해 이루어져야 하기 때문에 그 과정에서 많은 일자리가 생겨날 수밖에 없다. 데이터댐과 관련된 대표적 사례로는 최근 정부가 내년까지 조성하기로 발표한 국가 바이오 데이터 스테이션(data station)을 꼽을 수 있다. 국가 바이오 데이터스테이션이란 과학기술정보통신부를 비롯한 10개 부처가 R&D 사업을 통해 확보한 바이오 관련 데이터를 한곳에 모아 활용할 수 있는 데이터댐이다."
